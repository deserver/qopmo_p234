 ...
 * </p>
 */
public class CruceSegmentOld implements OperadorCruce {
		...
		while (iterador1.hasNext() && iterador2.hasNext()) {
			...
			for (Nodo next : iguales) {
				...
				if (subCamino == null) {
					...
					// "cruce_error_a");
					// Poblacion.getRed().drawServicio(gen2, dir,
					// "cruce_error_b");
				 ...
				 * nuevoPrimario.desbloquearCanales(); nuevoPrimario = null;
				 * break; }
				 */
				// se va anexando el camino creado.
				nuevoPrimario.anexar(subCamino);
				// luego de agregar, se bloquea los Canales (evitar repetir).
